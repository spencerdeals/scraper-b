...
// Scraper B — clean service: POST /scrape { url } -> { name, price, image, variants }
...
app.get("/health", (_req, res) => {
  res.json({ ok: true, service: "scraper-b", version: "1.0.1" });
});
...
async function scrapeAmazon(url) {
  ...
  let p = html.match(/"priceToPay"[^}]*"amount"\s*:\s*([0-9.]+)/i);
  ...
  // image
  let i = html.match(/"hiRes":"(https:[^"]+)"/i);
